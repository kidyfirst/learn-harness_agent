...
BLEND_MODES = {
    ...
    "vividlight": lambda a, b: np.where(
        b < 0.5, np.clip(1 - (1 - a) / (2 * b + 1e-6), 0, 1), np.clip(a / (2 * (1 - b) + 1e-6), 0, 1)
    ),
...
def fx_psychedelic(r, f, t, S):
    ...
    canvas_a = _render_vf(
        r, "md", lambda g, f, t, S: vf_plasma(g, f, t, S) * 1.3, hf_angle(0.0), PAL_DENSE, f, t, S, sat=0.8
    )
...
class FeedbackBuffer:
    ...
    def apply(self, canvas, decay=0.85, blend="screen", opacity=0.5, transform=None, transform_amt=0.02, hue_shift=0.0):
        ...
        result = blend_canvas(canvas, np.clip(self.buf * 255, 0, 255).astype(np.uint8), blend, opacity)
    ...
    def _transform(self, buf, transform, amt):
        ...
        if transform == "zoom":
            ...
            if m > 0 and n > 0:
                ...
                buf = (
                    np.array(
                        Image.fromarray(np.clip(cropped * 255, 0, 255).astype(np.uint8)).resize((w, h), Image.NEAREST)
                    ).astype(np.float32)
                    / 255.0
                )
    ...
    def _hue_shift(self, buf, amount):
        ...
        delta = mx - mn + 1e-10
        # Hue
        h = np.where(mx == r, ((g - b) / delta) % 6, np.where(mx == g, (b - r) / delta + 2, (r - g) / delta + 4))
        h = (h / 6 + amount) % 1.0
...
```python
def mask_iris(g, t, t_start, t_end, cx_frac=0.5, cy_frac=0.5, max_radius=0.7, ease_fn=None):
...
def apply_mask_canvas(canvas, mask, bg_canvas=None):
    ...
    mask_px = np.repeat(
        np.repeat(mask, canvas.shape[0] // mask.shape[0] + 1, axis=0), canvas.shape[1] // mask.shape[1] + 1, axis=1
    )
    mask_px = mask_px[: canvas.shape[0], : canvas.shape[1]]

    if bg_canvas is not None:
        return np.clip(canvas * mask_px[:, :, None] + bg_canvas * (1 - mask_px[:, :, None]), 0, 255).astype(np.uint8)
