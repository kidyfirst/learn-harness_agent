...
```python
def bg_sinefield(
    g, f, t, hue=0.6, bri=0.5, pal=PAL_DEFAULT, freq=(0.13, 0.17, 0.07, 0.09), speed=(0.5, -0.4, -0.3, 0.2)
):
...
```python
def update_boids(
    S, g, f, n_boids=200, perception=8.0, max_speed=2.0, sep_weight=1.5, ali_weight=1.0, coh_weight=1.0, char_set=None
):
...
```python
def update_flow_particles(S, g, f, flow_field, n=500, speed=1.0, life_drain=0.005, emit_rate=10, char_set=None):
...
def draw_particle_trails(S, g, trail_key="trails", max_trail=8, fade=0.7):
    ...
    current = list(zip([int(y) for y in S.get("py", [])], [int(x) for x in S.get("px", [])]))
...
```python
def eff_matrix_rain(g, f, t, S, hue=0.33, bri=0.6, pal=PAL_KATA, speed_base=0.5, speed_beat=3.0):
...
def eff_glitch_displace(ch, co, f, intensity=1.0):
    ...
    for _ in range(int(n_bands)):
        ...
        shift = int(
            (random.random() - 0.5) * f.get("rms", 0.3) * 40 + f.get("bdecay", 0) * 20 * (random.random() - 0.5)
        )
        if shift != 0:
            for row in range(h):
                rr = y + row
...
def eff_shake(ch, co, f, x_amp=6, y_amp=3):
    shake_x = int(
        f.get("sub", 0.3) * x_amp * (random.random() - 0.5) * 2 + f.get("bdecay", 0) * 4 * (random.random() - 0.5) * 2
    )
...
```python
def vf_sinefield(g, f, t, S, bri=0.5, freq=(0.13, 0.17, 0.07, 0.09), speed=(0.5, -0.4, -0.3, 0.2)):
...
def vf_fbm(g, f, t, S, octaves=5, freq=0.06, lacunarity=2.0, gain=0.5, speed=0.2, bri=0.8):
...
def vf_domain_warp(g, f, t, S, base_fn=None, warp_fn=None, warp_strength=15.0, freq=0.06, speed=0.2):
    ...
    if base_fn is not None:
        ...
        val = _value_noise_2d(warped_cc * freq * 0.8 + t * speed * 0.5, warped_rr * freq * 0.7 - t * speed * 0.3)
    else:
        ...
        for i in range(4):
            val = (
                val
                + _value_noise_2d(
                    warped_cc * fx + t * speed * 0.5 + i * 13.7, warped_rr * fy - t * speed * 0.3 + i * 27.3
                )
                * amp
            )
            amp *= 0.5
...
def vf_voronoi(g, f, t, S, n_cells=20, speed=0.3, edge_width=1.5, mode="distance", seed=42):
...
def vf_reaction_diffusion(
    g, f, t, S, feed=0.055, kill=0.062, da=1.0, db=0.5, dt=1.0, steps_per_frame=8, init_mode="spots"
):
...
def vf_game_of_life(
    g, f, t, S, rule="life", birth=None, survive=None, steps_per_frame=1, density=0.3, fade=0.92, seed=42
):
...
def vf_strange_attractor(g, f, t, S, attractor="clifford", n_points=50000, warmup=500, bri=0.8, seed=42, params=None):
...
def vf_sdf(g, f, t, S, sdf_fn=sdf_circle, edge_width=1.5, glow=False, glow_falloff=0.03, animate=True, **sdf_kwargs):
...
def hf_gradient_v(start=0.0, end=1.0):
    ...
    def fn(g, f, t, S):
        h = np.broadcast_to(start + (g.rr / g.rows) * (end - start), (g.rows, g.cols)).copy()
        return h % 1.0
...
def ease_elastic(t):
    ...
    return np.where(t == 0, 0, np.where(t == 1, 1, 2 ** (-10 * t) * np.sin((t * 10 - 0.75) * (2 * np.pi) / 3) + 1))
...
def vf_sequence(g, f, t, S, fields, durations, crossfade=1.0, ease_fn=ease_in_out_cubic):
...
def vf_temporal_noise(g, f, t, S, freq=0.06, t_freq=0.3, octaves=4, bri=0.8):
    ...
    fx = freq
    for i in range(octaves):
        val_lo = val_lo + _value_noise_2d(g.cc * fx + t_lo * 7.3 + i * 13, g.rr * fx + t_lo * 3.1 + i * 29) * amp
        val_hi = (
            val_hi + _value_noise_2d(g.cc * fx + (t_lo + 1) * 7.3 + i * 13, g.rr * fx + (t_lo + 1) * 3.1 + i * 29) * amp
        )
        amp *= 0.5
        fx *= 2.0
...
def scene_complex(r, f, t, S):
    ...
    overlay = _render_vf(
        r.grids["sm"], vf_rings(r.grids["sm"], f, t, S), hf_fixed(0.6)(r.grids["sm"], f, t, S), pal=PAL_BLOCK
    )
