...
pointer.add_updater(
    lambda m: m.put_start_and_end_on(nl.n2p(tracker.get_value()) + UP * 0.5, nl.n2p(tracker.get_value()))
)
...
values = [5, 2, 8, 1, 9, 3]
bars = (
    VGroup(*[Rectangle(width=0.6, height=v * 0.4, color=BLUE, fill_opacity=0.7) for v in values])
...
stream = StreamLines(lambda pos: np.array([-pos[1], pos[0], 0]), stroke_width=2, max_anchors_per_line=30)
self.add(stream)
