...
name: plan
description: |
  Plan mode only — write an actionable markdown plan under `plans/`, no execution.
  Bite-sized tasks, exact paths, complete code. Trigger when the user wants a plan
...
- Delegating to subagents via task-based execution
...
> **For the agent:** Use task-based execution skill to implement this plan task-by-task.
...
Use harness filesystem tools to understand the project:
...
**"Plan complete and saved. Ready to execute with the `task` tool — one subagent per plan task. Shall I proceed?"**

When executing:
- Fresh `task` dispatch per plan task with full context from the saved plan
- Review output before moving to the next task
