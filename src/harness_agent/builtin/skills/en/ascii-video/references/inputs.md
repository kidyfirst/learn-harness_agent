...
subprocess.run(
    ["ffmpeg", "-y", "-i", input_path, "-ac", "1", "-ar", "22050", "-sample_fmt", "s16", tmp],
    capture_output=True,
    check=True,
)
...
def build_tts_track(tts_clips, target_duration, intro_pad=5.0, outro_pad=4.0):
    ...
    for clip in tts_clips:
        ...
        subprocess.run(
            ["ffmpeg", "-y", "-i", clip, "-ac", "1", "-ar", str(sr), "-sample_fmt", "s16", wav],
            capture_output=True,
            check=True,
        )
...
def render_typewriter(ch, co, lines, block_start, cols, progress, total_chars, text_rgb, t):
    ...
    if progress < 1.0 and int(t * 3) % 2 == 0:
        ...
        for li, line in enumerate(lines):
            for ci, c in enumerate(line):
                if cc == chars_visible:
                    stamp(ch, co, "\u258c", block_start + li, (cols - len(line)) // 2 + ci, (255, 220, 100))
                    return
                cc += 1
```
