...
parser.add_argument(
    "--quality", choices=["draft", "preview", "production", "max", "auto"], default="auto", help="Render quality preset"
)
parser.add_argument(
    "--aspect", choices=["landscape", "portrait", "square"], default="landscape", help="Aspect ratio preset"
)
...
def render_bg(self, color_field, palette=PAL_CIRCUIT):
    ...
    color_px = color_field[self._bg_row_idx[:, None], self._bg_col_idx[None, :]].astype(np.float32)
...
for fi in range(n_cols):
    ...
    for dx in range(-1, 2):  # 3-wide columns
        c = fx_c + dx
        if 0 <= c < cols:
            fire_val[fr, c] = np.maximum(fire_val[fr, c], (1 - frac * 0.6) * (0.5 + rms * 0.5))
...
with ProcessPoolExecutor(max_workers=N_WORKERS) as pool:
    futures = {pool.submit(render_clip, seg, features, path): seg["id"] for seg, path in clip_args}
...
for t in [2, 30, 60, 120, 180]:
    cmd = ["ffmpeg", "-ss", str(t), "-i", output_path, "-frames:v", "1", "-f", "rawvideo", "-pix_fmt", "rgb24", "-"]
