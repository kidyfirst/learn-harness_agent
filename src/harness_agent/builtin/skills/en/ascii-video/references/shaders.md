...
class FeedbackBuffer:
    ...
    def apply(self, canvas, decay=0.85, blend="screen", opacity=0.5, transform=None, transform_amt=0.02, hue_shift=0.0):
...
fb_cfg = {"decay": 0.8, "blend": "screen", "opacity": 0.4, "transform": "zoom", "transform_amt": 0.015}
...
# Ghostly echo (horror)
fb_cfg = {"decay": 0.9, "blend": "add", "opacity": 0.15, "transform": "shift_up", "transform_amt": 0.01}
...
# Rising heat haze
fb_cfg = {"decay": 0.5, "blend": "add", "opacity": 0.2, "transform": "shift_up", "transform_amt": 0.02}
```
...
def _apply_shader_step(canvas, name, kwargs, f, t):
    ...
    if name == "crt":
        ...
        return sh_wave_distort(canvas, t, kwargs.get("freq", 0.02), kwargs.get("amp", 8), kwargs.get("axis", "x"))
    ...
    # --- Channel ---
        ...
        return sh_channel_shift(canvas, kwargs.get("r", (0, 0)), kwargs.get("g", (0, 0)), kwargs.get("b", (0, 0)))
    ...
    # --- Tone ---
        ...
        return sh_levels(canvas, kwargs.get("black", 0), kwargs.get("white", 255), kwargs.get("midtone", 1.0))
...
def sh_channel_shift(c, r_shift=(0, 0), g_shift=(0, 0), b_shift=(0, 0)):
    ...
    for ch_i, (sx, sy) in enumerate([r_shift, g_shift, b_shift]):
        if sx != 0:
            o[:, :, ch_i] = np.roll(c[:, :, ch_i], sx, axis=1)
        if sy != 0:
            o[:, :, ch_i] = np.roll(o[:, :, ch_i], sy, axis=0)
    return o
```
...
def sh_rgb_split_radial(c, strength=5):
    ...
    out = c.copy()
    ry = np.clip(Y.astype(int) + dy, 0, h - 1)
    rx = np.clip(X.astype(int) + dx, 0, w - 1)
    out[:, :, 0] = c[ry, rx, 0]  # red shifts outward
    by = np.clip(Y.astype(int) - dy, 0, h - 1)
    bx = np.clip(X.astype(int) - dx, 0, w - 1)
...
def sh_block_glitch(c, n_blocks=8, max_size=40):
    ...
    for _ in range(n_blocks):
        bw = random.randint(10, max_size)
        bh = random.randint(5, max_size // 2)
        sx = random.randint(0, w - bw - 1)
        sy = random.randint(0, h - bh - 1)
...
def tr_glitch_cut(ch_a, co_a, ch_b, co_b, blend):
    if blend < 0.5:
        ch, co = ch_a.copy(), co_a.copy()
    else:
        ch, co = ch_b.copy(), co_b.copy()
    if 0.3 < blend < 0.7:
        ...
        for _ in range(int(intensity * 20)):
            ...
            if shift:
                ch[y] = np.roll(ch[y], shift)
                co[y] = np.roll(co[y], shift, axis=0)
    return ch, co
```
...
cmd = [
    ...
    "pipe:0",
    "-vf",
    f"fps={fps},scale={W}:{H}:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse",
    "-loop",
    "0",
    output_gif,
]
...
def render_char_rgba(canvas, row, col, char_img, color_rgb, alpha=255):
    ...
    canvas[y0 : y0 + cell_h, x0 : x0 + cell_w, 0] = np.maximum(
        canvas[y0 : y0 + cell_h, x0 : x0 + cell_w, 0], (mask * r / 255).astype(np.uint8)
    )
    canvas[y0 : y0 + cell_h, x0 : x0 + cell_w, 1] = np.maximum(
        canvas[y0 : y0 + cell_h, x0 : x0 + cell_w, 1], (mask * g / 255).astype(np.uint8)
    )
    canvas[y0 : y0 + cell_h, x0 : x0 + cell_w, 2] = np.maximum(
        canvas[y0 : y0 + cell_h, x0 : x0 + cell_w, 2], (mask * b / 255).astype(np.uint8)
    )
...
def frame_to_ansi_delta(chars, colors, prev_chars, prev_colors):
    ...
    for r in range(rows):
        for c in range(cols):
            if chars[r, c] != prev_chars[r, c] or not np.array_equal(colors[r, c], prev_colors[r, c]):
