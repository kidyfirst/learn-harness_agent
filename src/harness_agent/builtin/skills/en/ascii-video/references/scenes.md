...
def fx_tunnel_ripple(r, f, t, S):
    ...
    canvas_b = _render_vf(
        ...
        "sm",
        lambda g, f, t, S: (
            vf_ripple(g, f, t, S, sources=[(0.3, 0.3), (0.7, 0.7), (0.5, 0.2)], freq=0.5, damping=0.012) * 1.4
        ),
        hf_angle(0.1),
        PAL_STARS,
        f,
    ...
    while i < len(S["px"]):
        ...
        if S["life"][i] <= 0:
            ...
            if 0 <= pr < g.rows and 0 <= pc < g.cols:
                ch_p[pr, pc] = S["pch"][i]
                co_p[pr, pc] = hsv2rgb_scalar(0.08 + (1 - S["life"][i]) * 0.15, 0.95, S["life"][i])
            i += 1
...
with ProcessPoolExecutor(max_workers=N_WORKERS) as pool:
    futures = {
        pool.submit(render_clip, seg, features, clip_path): seg["id"] for seg, clip_path in zip(segments, clip_paths)
    }
...
def fx_breathing_plasma(r, f, t, S):
    ...
    canvas = _render_vf(
        r, "md", lambda g, f, t, S: vf_plasma(g, f, t, S) * 1.3, hf_time_cycle(0.08), PAL_DENSE, f, t, S, sat=0.8
    )
    return canvas
```
...
def fx_voronoi_spiral(r, f, t, S):
    ...
    canvas_a = _render_vf(
        r,
        "md",
        lambda g, f, t, S: vf_voronoi(g, f, t, S, n_cells=15, mode="edge", edge_width=2.0, speed=0.4),
        hf_angle(0.2),
        PAL_CIRCUIT,
        f,
        t,
...
def fx_organic_warp(r, f, t, S):
    ...
    canvas_b = _render_vf(
        r,
        "md",
        lambda g, f, t, S: vf_domain_warp(g, f, t, S, warp_strength=20.0, freq=0.05, speed=0.15),
        hf_time_cycle(0.05),
        PAL_BRAILLE,
        f,
        t,
...
def fx_cathedral(r, f, t, S):
    ...
    canvas_c = _render_vf(
        r,
        "lg",
        lambda g, f, t, S: vf_temporal_noise(g, f, t, S, freq=0.04, t_freq=0.2, octaves=3),
        hf_time_cycle(0.12),
        PAL_BLOCKS,
        f,
        t,
...
def fx_masked_life(r, f, t, S):
    ...
    canvas_bg = _render_vf(
        r,
        "sm",
        lambda g, f, t, S: vf_strange_attractor(g, f, t, S, attractor="clifford", n_points=30000),
        hf_time_cycle(0.04),
        PAL_DOTS,
        f,
        t,
    ...
    rd_field = vf_reaction_diffusion(g_sm, f, t, S, feed=0.046, kill=0.063, steps_per_frame=0)  # read without stepping
    ch_p, co_p = update_flow_particles(S, g_sm, f, rd_field, n=300, speed=0.8, char_set=list("·•◦∘°"))
...
def fx_morphing_journey(r, f, t, S):
    ...
    twist = keyframe(t, [(0, 1.0), (5, 5.0), (10, 2.0), (15, 8.0), (20, 1.0)], ease_fn=ease_in_out_cubic, loop=True)
    ...
    val_fn = lambda g, f, t, S: vf_sequence(g, f, t, S, fields, durations, crossfade=2.0)

    # Render with slowly rotating hue
    canvas = _render_vf(r, "md", val_fn, hf_time_cycle(0.06), PAL_DENSE, f, t, S, sat=0.8)

    # Second layer: tiled version of same sequence at smaller grid
    tiled_fn = lambda g, f, t, S: vf_sequence(
        make_tgrid(g, *uv_tile(g, 3, 3, mirror=True)), f, t, S, fields, durations, crossfade=2.0
    )
...
def fx_life(r, f, t, S):
    ...
    canvas = _render_vf(
        r,
        "sm",
        lambda g, f, t, S: vf_game_of_life(g, f, t, S, rule="life", steps_per_frame=1, fade=0.92, density=0.25),
        hf_fixed(0.33),
        PAL_BLOCKS,
        f,
        t,
    ...
    canvas_b = _render_vf(
        ...
        lambda g, f, t, S: vf_game_of_life(
            g, f, t, S, rule="coral", steps_per_frame=1, fade=0.85, density=0.15, seed=99
        ),
        hf_time_cycle(0.1),
        PAL_HATCH,
        f,
...
def fx_boid_swarm(r, f, t, S):
    ...
    canvas_bg = _render_vf(
        r,
        "md",
        lambda g, f, t, S: vf_voronoi(g, f, t, S, n_cells=20, mode="distance", speed=0.2),
    ...
    # Foreground: boids
    g = r.get_grid("md")
    ch_b, co_b = update_boids(S, g, f, n_boids=150, perception=6.0, max_speed=1.5, char_set=list("▸▹►▻→⟶"))
...
def fx_fire_text(r, f, t, S):
    ...
    canvas_bg = _render_vf(
        r,
        "md",
        lambda g, f, t, S: vf_domain_warp(g, f, t, S, warp_strength=8, freq=0.03, speed=0.05) * 0.3,
        hf_fixed(0.6),
        PAL_DENSE,
        f,
        t,
...
def fx_portrait_rain_quote(r, f, t, S):
    ...
    ch, co, S = eff_matrix_rain(g, f, t, S, hue=0.33, bri=0.6, pal=PAL_KATA, speed_base=0.4, speed_beat=2.5)
    ...
    quote_lines = layout_text_portrait("The code is the art and the art is the code", max_chars_per_line=20)
    # Center vertically
    ...
    if "_scene_start" not in S:
        S["_scene_start"] = t
    render_typewriter(ch_t, co_t, quote_lines, block_start, g_text.cols, progress, total_chars, (200, 255, 220), t)
...
SCENES = [
    {
        ...
        "feedback": {"decay": 0.8, "blend": "screen", "opacity": 0.3, "transform": "zoom", "transform_amt": 0.01},
    },
    {
        "start": 5.0,
        "end": 15.0,
