...
**Action:** Use `read_file` on the relevant source files. Use `glob` to find the error string in the codebase.
...
**Action:** Use the `execute` tool to run the tight loop:
...
**Action:** Use `glob` to trace references:
...
**Action:** Use `glob` to find comparable patterns:
...
- **`grep`** — Find error strings and trace references in source
- **`glob`** — Locate files by name or pattern
- **`read_file`** — Read source with line numbers for precise analysis
- **`execute`** — Run tests, check git history, reproduce bugs
- **`web_fetch`** — Research error messages and library docs
