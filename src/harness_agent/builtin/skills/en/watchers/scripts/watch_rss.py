...
def _parse_feed(xml_bytes: bytes):
    ...
    for item in root.iter():
        ...
        href = link_el.attrib.get("href") or (link_el.text or "").strip() if link_el is not None else ""
...
def main() -> int:
    ...
    p.add_argument("--max", type=int, default=10, help="Max new items to emit per tick (default: 10)")
    p.add_argument(
        "--with-summary", action="store_true", help="Include <description>/<summary> snippet under each item"
    )
