...
def _state_dir() -> Path:
    ...
    # Default: <workspace>/watcher-state/ (execute cwd is usually the workspace root).
    workspace = os.environ.get("HARNESS_WORKSPACE_DIR") or os.getcwd()
    return Path(workspace) / "watcher-state"
...
class Watermark:
    ...
    def filter_new(self, items: Iterable[dict[str, Any]], *, id_key: str = "id") -> list[dict[str, Any]]:
