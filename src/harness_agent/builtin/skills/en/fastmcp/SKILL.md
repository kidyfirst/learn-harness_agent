...
- smoke-test a server with the FastMCP CLI before wiring it into harness-agent or another MCP client
...
Use the `mcporter` skill when the server already exists and only needs CLI access. For harness-agent runtime integration, add the server to `mcp_server_configs` in your agent config.
...
When the goal is harness-agent integration, add the server to `mcp_server_configs` in your deployment config, or keep using FastMCP CLI commands during development until the interface stabilizes.
...
### harness-agent cannot see the deployed server

The server-building part may be correct while the agent MCP config is not. Add the server to `mcp_server_configs` and restart the agent process.
