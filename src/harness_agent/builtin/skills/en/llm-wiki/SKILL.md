...
**Location:** Set via `WIKI_PATH` environment variable (e.g. in `{workspace}/.env`).
...
For large wikis (100+ pages), also run a quick `glob` for the topic
...
① **Capture the raw source:**
   ...
   - PDF → use `web_fetch` (handles PDFs), save to `raw/papers/`
...
③ **Check what already exists** — search index.md and use `glob` to find
...
② **For wikis with 100+ pages**, also `glob` across all `.md` files
